...
class Smartcrawl_Check_Imgalts_Keywords extends Smartcrawl_Check_Abstract {
	...
	public function get_status_msg() {
		...
		if ( $this->_state ) {
			$message = esc_html__( "Image alts", 'wds' );
		} else if ( 0 === $image_count ) {
			$message = esc_html__( "You haven't added any images", 'wds' );
		} else {
			$percentage = $this->get_percentage();
			if ( $percentage > 75 ) {
				$message = esc_html__( 'Too many of your image alt texts contain the focus keyword', 'wds' );
			} elseif ( $percentage === 0 ) {
				$message = esc_html__( 'Your image alt texts have no keywords', 'wds' );
			} else {
				$message = esc_html__( 'Too few of your image alt texts contain the focus keyword', 'wds' );
			}
		}
	...
	public function get_recommendation() {
		...
		if ( $this->_state ) {
			...
			if ( $percentage > 75 ) {
				$message = sprintf(
					esc_html__( '%d/%d images on this page have alt text with your keyword which is too much. Whilst it’s great that you have image alternative text with your focusd keyword(s), you can also get penalized for having too many keywords on a page. Try to include your keywords in image alt texts only when it makes sense.', 'wds' ),
