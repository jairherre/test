...
class Cartflows_Pro_Checkout_Meta {
	...
	public function tab_content_checkout_general( $options, $post_id ) {
		...
		if ( ! cartflows_pro_is_active_license() ) {
			...
			echo wcf()->meta->get_description_field(
				array(
					...
					/* translators: %s: link */
					'content' => '<i>' . sprintf( esc_html__( 'Activate %1$sCartFlows Pro%2$s license to access Pre-applied Coupon, Product Variations & Quantity Options.', 'cartflows-pro' ), '<a href="' . CARTFLOWS_PRO_LICENSE_URL . '" target="_blank">', '</a>' ) . '</i>',
				)
			);
		...
		echo wcf()->meta->get_select_field(
			array(
				'label'   => __( 'Your Products Position', 'cartflows-pro' ),
