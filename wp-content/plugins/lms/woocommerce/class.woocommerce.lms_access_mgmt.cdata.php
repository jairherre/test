...
class LMS_Woocommerce_AccessMgmt_Cdata {
	
	public function __construct(){
		if(get_option('lms_woocommerce_enabled')){
			add_action( 'add_meta_boxes_lms_access_mgmt', array( $this, 'woocommerce_access_mgmt_cdata' ) );
		}
	...
	public function woocommerce_access_mgmt_cdata_callback( $post ) {
		...
		echo '<p><strong>'.__('Woocommerce Product ID','lms').'</strong></p>';
		echo '<p><input type="text" name="woocommerce_access_mgmt_id_product" value="'.$woocommerce_access_mgmt_id_product.'" class="widefat"><br><i>'.__('Enter Product ID from','lms').' <a href="edit.php?post_type=product" target="_blank">'.__('Woocommerce Products','lms').'</a></i></p>';

	}
	
}

return new LMS_Woocommerce_AccessMgmt_Cdata();
