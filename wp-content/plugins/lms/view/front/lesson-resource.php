...
if ( ! empty( $resource_data ) ) : ?>
	...
	<div class="lms-ResourcesSections">
		...
		foreach ( $resource_data as $key => $value ) :
			if ( isset( $value->r_id) && $value->r_id == 0 ) :
				...
				echo '<div class="lms-ResourcesSection '.($value->highlight == 'yes' ? 'lms-ResourcesSection--highlight':'').'">';
				$i++;
				?>

				<h3 class="lms-ResourcesSection__title"><?php echo stripslashes($value->title);?></h2>

			<?php else : ?>
				<ul class="lms-ResourcesList">
				<?php

				if ( isset( $value->link_url ) && $value->link_url != '' ) : // this is a single resource
					?>
					<li class="lms-ResourcesList__item"><?php echo get_resource_icon_fa(($value->link_url == 'yes'?'.ext_link':'.link'));?> <a class="lms-ResourcesList__itemLink" href="<?php echo $value->link_url;?>" target="<?php echo ($value->link_open_in_new_tab == 'yes'?'_blank':'');?>" rel="<?php echo ($value->link_nofollow == 'yes'?'nofollow':'');?>"><?php echo $value->title;?></a></li>
					<?php
				else :
