	...
	public function __construct( $cls ){		
		if( $_FILES['import_data']['name']){
			if( in_array($_FILES["import_data"]["type"],$this->allowed_file_types) ){
				...
				include_once LMS_DIR_PATH . '/excel_import/vendor/php-excel-reader/excel_reader2.php';
				include_once LMS_DIR_PATH . '/excel_import/vendor/SpreadsheetReader.php';
				
				//echo '<pre>';
				
				$upload_dir = wp_upload_dir();
				$file_path = $upload_dir['basedir'] . '/' . 'lms' . '/' . $_FILES['import_data']['name'];
				...
				for( $i = 0; $i < $sheetcount; $i++ ){
					...
					foreach ($reader as $row){
						...
						if($course_title){
							$is_course_exists = $wpdb->get_row( $wpdb->prepare( "SELECT ID FROM " . $wpdb->posts . " WHERE post_title = %s AND post_type = %s",  $course_title, 'lms_course' ) );
							if( !$is_course_exists ){
								$new_course = array(
						...
						if($section_title){
							$is_section_exists = $wpdb->get_row( $wpdb->prepare( "SELECT s_id FROM " . $wpdb->prefix . "lms_section WHERE s_name = %s AND c_id = %d",  $section_title, $course_id ) );
							if( !$is_section_exists ){
								$section_data = array( 'c_id' => $course_id, 's_name' => $section_title );
								$wpdb->insert( $wpdb->prefix.'lms_section', $section_data );
								$section_id = $wpdb->insert_id;
								
								// mapping //
								$lesson_mapping_data = array( 'c_id' => $course_id, 'l_id' => 0, 'm_type' => 'section', 's_id' => $section_id, 'm_order' => $order, 'l_delay' => 0 );
								$wpdb->insert( $wpdb->prefix.'lms_lesson_mapping', $lesson_mapping_data );
								$order++;
								// mapping //
						...
						if($lesson_title){
							$is_lesson_exists = $wpdb->get_row( $wpdb->prepare( "SELECT ID FROM " . $wpdb->posts . " WHERE post_title = %s AND post_type = %s",  $lesson_title, 'lms_lesson' ) );
							if( !$is_lesson_exists ){
								$new_lesson = array(
						...
						if($resource_box_title){
							$is_resource_box_exists = $wpdb->get_row( $wpdb->prepare( "SELECT m_id FROM " . $wpdb->prefix . "lms_resource_mapping WHERE r_title = %s AND l_id = %d",  $resource_box_title, $lesson_id ) );
							if( !$is_resource_box_exists ){
								$resource_box_data = array( 'l_id' => $lesson_id, 'r_id' => 0, 'r_title' => $resource_box_title, 'r_highlight' => 'no' );
								$wpdb->insert( $wpdb->prefix.'lms_resource_mapping', $resource_box_data );
						...
						if($resource_title){
							$is_resource_exists = $wpdb->get_row( $wpdb->prepare( "SELECT ID FROM " . $wpdb->posts . " WHERE post_title = %s AND post_type = %s",  $resource_title, 'lms_resource' ) );
									...
									if(is_array($links)){
										foreach($links as $key => $value){
											...
											// mapping //
												...
												$wpdb->insert( $wpdb->prefix.'lms_resource', $resource_mapping_data );
												$resource_m_id = $wpdb->insert_id;
											// mapping //
		...
		if(isset($_REQUEST['import_data_gs']) and $_REQUEST['import_data_gs'] != ''){
			...
			$file_path = $upload_dir['basedir'] . '/' . 'lms' . '/' . basename($gs_file);
