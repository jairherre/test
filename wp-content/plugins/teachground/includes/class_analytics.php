...
class tg_analytics_class {
   ...
   public  function __construct(){
	  ...
	  $this->table = 'tg_orders';
    }
	
	public function get_table_colums(){
		$colums = array(
		'first_name' => __('Firstname','teachground'),
		'last_name' => __('Lastname','teachground'),
		'email' => __('Email','teachground'),
		'last_login' => __('Last Login','teachground'),
		'access' => __('Access Name','teachground'),
		'gateway' => __('Gateway','teachground'),
		'order_id' => __('Order ID','teachground'),
		'order_date' => __('Order Date','teachground'),
		);
	...
	public function lists(){
		...
		echo '<h3>' . __('Analytics','teachground') . '</h3>';
    ...
    public function display_list() {
		...
		if(isset($_REQUEST['action']) and $_REQUEST['action'] == 'login_log'){
			...
			$lll = new tg_analytics_login_log_class;
			...
			$laml = new tg_analytics_access_log_class;
	...
	public function get_access_names( $data ){
		...
		$am_ids = tg_get_accesses_of_user($user_id);
		$ret = '';
		if( is_array($am_ids) and count($am_ids) ){
			foreach($am_ids as $value){
				$ret .= '<p><a href="'.admin_url('edit.php?post_type=tg_course&page='.$this->plugin_page_base.'&action=access_log&am_id='.$value.'&user_id='.$user_id).'">' . get_the_title($value) . '</a></p>';
			}
		} else {
			$ret .= __('No accesses found','teachground');
		}
		return $ret;
	}
	...
	public function get_last_login_data( $user_id ){
		global $wpdb;
		$query = $wpdb->prepare( "SELECT l_added FROM ".$wpdb->prefix."tg_login_log WHERE user_id = %d AND l_status = %s ORDER BY l_added DESC LIMIT 1", $user_id, 'success' );
		$result = $wpdb->get_row( $query );
		if($result){
			return '<a href="'.admin_url('edit.php?post_type=tg_course&page='.$this->plugin_page_base.'&action=login_log&user_id='.$user_id).'">'.$result->l_added.'</a>';
		} else {
			return 'NA';
		}
	}
	...
	public function get_order_id_with_link( $order_id = '', $payment_gateway = '' ){
		if( $payment_gateway == 'woocommerce' ){
			...
			if( get_option('tg_thrivecart_id') ){
				return '<a href="https://thrivecart.com/'.get_option('tg_thrivecart_id').'/#/orders/view/'.$order_id.'/live/charges" target="_blank">'.$order_id.'</a>';
			} else {
				return $order_id;
			}
