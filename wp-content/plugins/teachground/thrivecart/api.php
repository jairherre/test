...
function thrivecart_tg_post_api_process(){
	
	if( isset( $_REQUEST['thtgapi'] ) and $_REQUEST['thtgapi'] == 'true' ){
		...
		$tg_thrivecart_secret = get_option( 'tg_thrivecart_api_key' );
		
		if( $thrivecart_secret == $tg_thrivecart_secret ){
			...
			switch ($action) {
				case 'order.success':
					...
					// get mapped AM from tg for upsell / downsell //
					if( $product_count == 2 ){
					...
					/*if( is_array( $tg_access_mgmt_data_ids ) and count($tg_access_mgmt_data_ids) ){
						foreach( $tg_access_mgmt_data_ids as $value ){
							$course_ids = tg_get_courses_from_am_id($value);
							if(is_array($course_ids)){
								foreach( $course_ids as $cid ){
									$tg_course_data_ids[] = $cid;
								}
							}
						}
					}*/
					...
					// get mapped courses //
					/*if( !empty($user_id) ){
						if( is_array( $tg_course_data_ids ) and count($tg_course_data_ids) ){
							foreach( $tg_course_data_ids as $value ){
								//ltg_enroll_student( $user_id, $value, 'api' );
								tg_assign_user_to_course( array('user_id' => $user_id, 'course_id' => $value, 'status' => 'Active' ) );
					...
					// add order log //
						tg_add_order( array( 'user_id' => $user_id, 'payment_gateway' => 'thrivecart', 'payment_gateway_order_id' => $response['order_id'] ) );
					// add order log //
				...
				case 'order.refund':
					...
					if ( email_exists( $user_email ) ) {
						...
						// delete order log //
						tg_delete_order( array( 'user_id' => $user_id, 'payment_gateway' => 'thrivecart', 'payment_gateway_order_id' => $response['order_id'] ) );
						// delete order log //
